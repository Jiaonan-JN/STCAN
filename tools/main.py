...
from lib.models.a2net import A2Net
...
from lib.utils.utils import save_model, backup_codes
import pdb
...
def main(subject, config):
    ...
    def warm_up_with_cosine_lr(epoch):
        ...
        if epoch <= cfg.TRAIN.WARM_UP_EPOCH:
            # 热身阶段：学习率从接近零线性增加到初始值
            return epoch / cfg.TRAIN.WARM_UP_EPOCH
        else:
            # 余弦衰减阶段：学习率平滑衰减
            progress = (epoch - cfg.TRAIN.WARM_UP_EPOCH) / (cfg.TRAIN.END_EPOCH - cfg.TRAIN.WARM_UP_EPOCH)
            cosine_decay = math.cos(progress * math.pi)
            return 0.5 * (cosine_decay + 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, help='experiment config file', default='/Users/zhangjiaonan/Downloads/Projects/LGSNet-main/LGSNet/experiments/cas.yaml')
